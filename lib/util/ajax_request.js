...
shaka.util.AjaxRequest.prototype.sendInternal = function() {
  ...
  if (Object.keys(this.parameters.requestHeaders).length > 0) {
    this.xhr_.setRequestHeader('Authorization', shaka.util.Auth.get_header());
  }
...
shaka.util.AjaxRequest.prototype.onLoad_ = function(event) {
  ...
  if (this.xhr_.status >= 200 && this.xhr_.status <= 299) {
    ...
    this.destroy_();
  } else if (this.xhr_.status == 401) {
    shaka.util.Auth.setToken();
